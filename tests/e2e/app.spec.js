...
test('End-to-end user workflow', async () => {
    ...
    const newTodoItem = window.locator('.todo-item').last();
    await expect(newTodoItem).toContainText(taskText);
    ...
    const checkbox = newTodoItem.locator('input[type="checkbox"]');
    ...
    const deleteButton = newTodoItem.locator('.delete-btn');
